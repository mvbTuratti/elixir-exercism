defmodule NewPassport do
  @type return() :: {:ok, String.t()} | {:error, String.t()} | {:retry, %NaiveDateTime{}}
  @spec get_new_passport(%NaiveDateTime{}, %NaiveDateTime{}, atom()) ::  return()
  def get_new_passport(now, birthday, form) do
    with {:ok, time_stamp} <- enter_building(now),
          {:ok, manual} <- find_counter_information(now),
          counter <- manual.(birthday),
          {:ok, checksum} <- stamp_form(time_stamp, counter, form),
          string <- get_new_passport_number(time_stamp, counter, checksum) do
      {:ok, string}
    else
      {:error, "city office is closed"} -> {:error, "city office is closed"}
      {:coffee_break, _instructions} -> {:retry, NaiveDateTime.add(now, 60*15)}
      {:error, msg} -> {:error, msg}
    end
